...
# registration validator using REGEX
    @staticmethod 
    def validate_registration(data):
        is_valid = True
        if len(data['first_name']) < 2:
            flash(" Your first name must be atleast 2 characters!","error_registration_first_name")
            is_valid = False
        if len(data['last_name']) < 2:
            flash(" Your last name must be atleast 2 characters!","error_registration_last_name")
            is_valid = False
